(function($) {
	...
	/**
	 * autor: CTAPbIu_MABP email: ctapbiumabp@gmail.com site:
	 * http://mabp.kiev.ua/content/2008/04/08/autocomplete/ license: MIT & GPL
	 * last update: 14.09.2009 version: 1.3
	 */
	...
	ac.prototype = {
		...
		init : function(ac, options) {
			...
			self.container = $('<div/>').css({
				width : self.width
			}).addClass('ac_conteiner').insertBefore(ac);
			self.ac = $(ac).attr({
				autocomplete : 'off'
			}).blur(function() {
			...
			.css({
				width : self.width - 22
			}) // 18 img + 2 margin + 2 border
			.addClass('ac_input').appendTo(self.container);
			self.img = $('<div/>').click(function() {
				clearTimeout(self.close);
				self.scroll();
				self.ul.toggle();
				self.ac.focus();
			}).addClass('ac_img').appendTo(self.container);
			self.ul = $('<div/>').addClass('ac_results').appendTo(
			...
			$(window).bind(
					...
					function() {
						...
						self.ul.css({
							width : self.width,
							top : (self.top ? c.offset().top - self.ul.height()
									- parseInt(c.css('border-top-width')) : c
									.offset().top
									+ c.height()
									+ parseInt(c.css('border-bottom-width'))),
							left : (c.offset().left + parseInt(c
		...
		scroll : function() {
			...
			if (!current.length)
				return; // quick return
			var el = current.get(0), list = self.ul.get(0); // dont scroll after
															// click on document
															// :(
			...
			if (mask.length >= self.minchar) {
				...
				if (self.check(mask) && !$.isFunction(self.url))
					...
					$.ajax({
						...
						url : $.isFunction(self.url) ? self.url(self)
								: self.url,
			...
			for ( var item in array)
				if (!mask.indexOf(array[item].toLowerCase())) {
					for ( var word in self.cache[array[item]])
						if (!self.cache[array[item]][word].toLowerCase()
								.indexOf(mask))
		...
		prepare : function(xml, mask) {
			...
			if (!self.store[mask]) {
				if (select.context) // use selectbox or ajax result
					...
					// use array or array-like object
		...
		mark : function(text, mask) {
			return new RegExp('^' + mask, 'ig').test(text) ? '<div>'
					+ text.replace(new RegExp('^' + mask, 'ig'),
							function(mask) {
								return '<span class="ac_match">' + mask
										+ '</span>';
							}) + '</div>' : '';
		},
		...
		display : function(list) {
			...
			return self.ul.html(list).children('div').mouseover(
					function() {
						$(this).siblings().removeClass('ac_over').end()
								.addClass('ac_over');
	...
	$.fn.autocomplete = function(options) {
		this.each(function() {
			if (this.tagName == 'SELECT' && !this.multiple) {
				var select = $(this), input = $("<input type='text'/>")
						.addClass(select.attr('class')).attr('tabindex',
								select.attr('tabindex')).attr('id',
								'ac_' + select.attr('id'));

				select.after(input).hide(); // add text input and hide the
											// select

				$("label[for='" + select.attr('id') + "']").attr('for',
						'ac_' + select.attr('id'));
